...
public class SearchService {
    ...
    private final com.example.yfin.http.YahooApiClient yahooApiClient;

    private final com.example.yfin.service.cache.RedisCacheService level2Cache;
    private final com.example.yfin.service.news.NewsAggregator newsAggregator;
    ...
    public Mono<SearchResponse> search(String query, int count, String lang, String region) {
        ...
        return level2Cache.get(key, new com.fasterxml.jackson.core.type.TypeReference<com.example.yfin.model.SearchResponse>() {})
                .switchIfEmpty(
                        yahooApiClient.getJson(path, "/search")
                                .map(this::mapSearch)
                                .flatMap(res -> level2Cache.set(key, res, java.time.Duration.ofSeconds(15)).thenReturn(res))
                );
    }
    ...
    public Mono<SearchResponse> searchGoogle(String query, int count, String lang) {
        ...
        return level2Cache.get(key, new com.fasterxml.jackson.core.type.TypeReference<com.example.yfin.model.SearchResponse>() {})
                .switchIfEmpty(
                        newsAggregator.fetchAll(query, c, lg)
                                .map(items -> {
